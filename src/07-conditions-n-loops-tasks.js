...
function isCreditCardNumber(ccn) {
  const arr = `${ccn}`.split('').map((i) => i * 1);
  let sum = 0;
  const parity = arr.length % 2;
  for (let i = 0; i < arr.length; i += 1) {
    if (i % 2 !== parity) sum += arr[i];
    else if (arr[i] > 4) sum += 2 * arr[i] - 9;
    else sum += 2 * arr[i];
  }
  return sum % 10 === 0;
}
...
function getCommonDirectoryPath(pathes) {
  const path = pathes.map((a) => a.split(/(\/)/g));
  const arr1 = [];
  let arr2 = [];
  for (let j = 0; j < path[0].length; j += 1) {
    if (path[0][j] === path[1][j]) {
      arr1.push(path[0][j]);
    } else break;
  }
  path.splice(0, 2, arr1);
  arr2 = path.map((a) => a.join(''));
  if (arr2.length > 1) return getCommonDirectoryPath(arr2);
  return arr2;
}
